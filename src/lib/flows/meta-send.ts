...
export async function engineSendText(
  ...
  const provider = await getProviderForConversation(db, args.conversationId)
  if (!provider) {
...
async function sendInteractiveViaMeta(
  ...
  const provider = await getProviderForConversation(db, input.conversationId)
  if (!provider) {
    throw new Error('WhatsApp not configured for this account')
  }
  ...
  const attempt = async (phone: string): Promise<string> => {
    if (input.kind === 'buttons') {
      const r = await provider.sendButtons({
        to: phone,
    ...
    const r = await provider.sendList({
      to: phone,
      bodyText: input.bodyText,
