...
import { getProvider, loadDefaultInstance } from '@/lib/channels/factory'
...
export async function POST(request: Request) {
  try {
    ...
    // Broadcasts have no per-thread instance, so they go out on the
    // account's default connection (Meta or Evolution).
    const instance = await loadDefaultInstance(supabase, accountId)
    if (!instance) {
      return NextResponse.json(
        {
          error:
