...
export async function POST(request: Request) {
  try {
    ...
    // build the matching provider (Meta or Evolution). A reply always
    // leaves on the same number it arrived on (conversation.instance_id);
    // legacy threads fall back to the account's default connection.
    let instance = conversation.instance_id
      ? await loadInstanceById(supabase, conversation.instance_id)
      : null
    if (!instance) instance = await loadDefaultInstance(supabase, accountId)
    if (!instance) {
      return NextResponse.json(
    ...
    // Self-heal legacy CBC-encrypted Meta tokens (idempotent, fire-and-
    // forget). Evolution keys are written GCM from the start.
    if (
      instance.provider === 'meta' &&
      instance.access_token &&
      isLegacyFormat(instance.access_token)
    ) {
      const upgraded = encrypt(decrypt(instance.access_token))
      void supabase
    ...
    const attempt = async (phone: string): Promise<string> => {
      ...
      if (isMediaKind) {
        ...
        // the provider). filename surfaces in the recipient's chat for
        // documents only.
    ...
    // Update conversation. Heal legacy threads that predate instance_id
    // by stamping the connection we just resolved.
    await supabase
      .from('conversations')
      .update({
        ...
        ...(conversation.instance_id ? {} : { instance_id: instance.id }),
      })
      .eq('id', conversation_id)
