...
export async function POST(request: Request) {
  try {
    ...
    const { data: conversation, error: convError } = await supabase
      .from('conversations')
      .select('id, account_id, instance_id, contact:contacts(phone)')
    ...
    // Resolve the connection this thread uses, then react via its provider.
    let instance = conversation.instance_id
      ? await loadInstanceById(supabase, conversation.instance_id as string)
      : null;
    ...
    try {
      ...
      console.error('[whatsapp/react] reaction send failed:', message);
      return NextResponse.json(
